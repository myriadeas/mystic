 ...
 */
public class SafeHtmlTreeBuilder extends TreeBuilder {

    private SafeHtmlTreeBuilderState state; // the current state
    private SafeHtmlTreeBuilderState originalState; // original / marked state
    ...
    public SafeHtmlTreeBuilder() {}

    @Override
    Document parse(String input, String baseUri, ParseErrorList errors) {
        state = SafeHtmlTreeBuilderState.Initial;
    ...
    boolean process(Token token, SafeHtmlTreeBuilderState state) {
        currentToken = token;
        return state.process(token, this);
    }

    void transition(SafeHtmlTreeBuilderState state) {
        this.state = state;
    }

    SafeHtmlTreeBuilderState state() {
        return state;
    }
    ...
    SafeHtmlTreeBuilderState originalState() {
    ...
    void error(SafeHtmlTreeBuilderState state) {
    ...
    void resetInsertionMode() {
        ...
        while (it.hasNext()) {
            ...
            if ("select".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InSelect);
                break; // frag
            } else if (("td".equals(name) || "td".equals(name) && !last)) {
                transition(SafeHtmlTreeBuilderState.InCell);
                break;
            } else if ("tr".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InRow);
                break;
            } else if ("tbody".equals(name) || "thead".equals(name) || "tfoot".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InTableBody);
                break;
            } else if ("caption".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InCaption);
                break;
            } else if ("colgroup".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InColumnGroup);
                break; // frag
            } else if ("table".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InTable);
                break;
            } else if ("head".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InBody);
                break; // frag
            } else if ("body".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InBody);
                break;
            } else if ("frameset".equals(name)) {
                transition(SafeHtmlTreeBuilderState.InFrameset);
                break; // frag
            } else if ("html".equals(name)) {
                transition(SafeHtmlTreeBuilderState.BeforeHead);
                break; // frag
            } else if (last) {
                transition(SafeHtmlTreeBuilderState.InBody);
                break; // frag
